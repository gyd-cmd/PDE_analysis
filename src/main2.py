...
if __name__ == '__main__':
    ...
    for w in wavelength:
        ...
        for var in voltage:
            ...
            SiPM_dark.append(readSipm(filepath2+"/dark/dark.txt", var)[0])
            PDE.append(I_SiPM)
        plt.figure()
        plt.plot(voltage, PDE, 'r.-', label="With light")
        plt.plot(voltage, SiPM_dark, 'b.-', label='Dark')
        ax = plt.gca()
        x_major_locator = MultipleLocator(10)
        ax.xaxis.set_major_locator(x_major_locator)
        plt.xlabel("Voltage [V]")
        plt.ylabel("HPK Current [A]")
        plt.legend()
        plt.yscale("log")
        plt.savefig("../resluts/Ham/sipm/"+str(w)+"nm.png")
        del PDE
        del SiPM_dark
        i += 1
    i = 0
   ...
   # PDE results vs wavelength at 52V.
    for w in wavelength:
        ...
        i += 1
    plt.figure()
    plt.errorbar(wavelength, PDE/PDE[0], yerr=Error/PDE[0], fmt='.k')
