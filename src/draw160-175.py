...
I = np.array(readData("../data/sipm/stable/longMonit.txt", 2))
t = [i*10 for i in range(len(I))]
t = np.array(t)
boolList = veto("../data/sipm/stable/longMonit.txt")
print(I)
print(t)
new_I = []
new_t = []
...
plt.text(1500, 2e-11, "Fit function : "+"%.2f" %
         (p1[0]*1e11)+"e-11+"+"%.2f" % (p1[1]*1e15)+"e-15*t")
print(p1[1])
plt.legend()
plt.plot(new_t, yvals, "r-")
plt.ylabel("$I_{PD} [A]$")
